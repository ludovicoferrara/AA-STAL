...
def inference_ava(args, model, device, dataset, class_names=None, class_colors=None):
    ...
    save_dir = os.path.join(args.save_folder, args.dataset, args.version)
    os.makedirs(save_dir, exist_ok=True)
    
    if args.save:
        save_path = os.path.join(save_dir, 'video_clips')
        os.makedirs(save_path, exist_ok=True)

    # Inizializzazione del file CSV di log
    csv_path = os.path.join(save_dir, 'action_log.csv')
    
    with open(csv_path, mode='w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file, delimiter=',')
        # Scrittura dell'intestazione richiesta
        ...
        for index in range(args.start_index, len(dataset)):
            ...
            # Ottenimento del tempo di esecuzione effettivo per process mining real-time
            ...
            # Inizializzazione identificativo persona temporaneo per il frame corrente
            person_counter = 1
            ...
            for bbox in bboxes:
                ...
                if len(scores) > 0:
                    # LOGICA CSV: Costruisce la stringa con le azioni concatenate e scrive la riga
                    ...
                    csv_writer.writerow([current_time_str, actions_str, person_id])
                    person_counter += 1
            ...
            # Forza la scrittura sul disco per leggere i log in real-time esternamente
            csv_file.flush()
